namespace _10_GenericTypesCollections.Models
{
    public class BookManager
    {
        ...
        {
            Books.Add(book);
            if (!BooksByAuthor.ContainsKey(book.Author)) BooksByAuthor[book.Author] = new List<Book>();
        ...
        public Book SearchByTitle(string title)
        {
            ...
            {
                if (book.Title.ToLower() == title.ToLower()) return book; 
            }
            return null;
        }
        public List<Book> GetBooksByAuthor(string author)
        {
            if (BooksByAuthor.ContainsKey(author)) return BooksByAuthor[author];
        ...
        public void  ServeNextInQueue()
        {
            ...
            {var member = WaitingQueue.Dequeue(); Console.WriteLine($"Xidmet edilir: {member}");}
            else Console.WriteLine("Növbe boşdur.");
        }
        ...
        {
            if (RecentlyReturned.Count != 0) return RecentlyReturned.Peek();
            else return null;
        }
    }
}